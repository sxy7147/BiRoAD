...
class DenoiseActor(nn.Module):
    ...
    def __init__(self,
                 ...
                 no_hand_embed=False,
                 use_biroad=False,
                 biroad_update_mode="residual",
                 biroad_placement="full",
                 ):
        super().__init__()
        ...
        self.use_biroad = use_biroad
        self.biroad_update_mode = biroad_update_mode
        ...
        self.prediction_head = TransformerHead(
            ...
            nhand=nhand,
            no_hand_embed=no_hand_embed,
            use_biroad=use_biroad,
            biroad_update_mode=biroad_update_mode,
            biroad_placement=biroad_placement,
        )
...
class TransformerHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.no_hand_embed = no_hand_embed
        self.nhand = nhand
        self.use_biroad = use_biroad
        self.biroad_update_mode = biroad_update_mode
        if biroad_update_mode not in ("residual", "direct"):
            raise ValueError("biroad_update_mode must be 'residual' or 'direct'")
        valid_biroad_placements = {
            "early", "middle", "late", "middle_late", "full",
        }
        if biroad_placement not in valid_biroad_placements:
            raise ValueError(
                "biroad_placement must be one of "
                f"{sorted(valid_biroad_placements)}"
            )
        self.biroad_placement = biroad_placement
        ...
        if self.use_biroad:
            self.symmetric_mlp = nn.Sequential(
                nn.Linear(embedding_dim, embedding_dim),
                nn.ReLU(),
                nn.Linear(embedding_dim, embedding_dim)
            )
            self.antisymmetric_mlp = nn.Sequential(
                nn.Linear(embedding_dim, embedding_dim),
                nn.ReLU(),
                nn.Linear(embedding_dim, embedding_dim)
            )
            self.symmetric_proj = nn.Linear(embedding_dim, embedding_dim)
            self.antisymmetric_proj = nn.Linear(embedding_dim, embedding_dim)
    ...
    def apply_biroad(self, traj_feats):
        """Apply BiRoAD to paired arm features of shape (B, L, 2, C)."""
        left_features = traj_feats[:, :, 0, :]
        right_features = traj_feats[:, :, 1, :]

        mean_features = 0.5 * (left_features + right_features)
        relative_features = 0.5 * (left_features - right_features)

        symmetric_features = self.symmetric_mlp(mean_features)
        antisymmetric_features = 0.5 * (
            self.antisymmetric_mlp(relative_features)
            - self.antisymmetric_mlp(-relative_features)
        )

        shared_update = self.symmetric_proj(symmetric_features)
        role_update = self.antisymmetric_proj(antisymmetric_features)

        left_update = shared_update + role_update
        right_update = shared_update - role_update

        if self.biroad_update_mode == "direct":
            return torch.stack([left_update, right_update], dim=2)
        return torch.stack(
            [left_features + left_update, right_features + right_update], dim=2
        )

    def should_apply_biroad(self, stage):
        if not self.use_biroad:
            return False
        placement_stages = {
            "early": {
                "post_lang",
        ...
        return stage in placement_stages[self.biroad_placement]

    def apply_biroad_to_flat_features(self, flat_feats, traj_len, nhand):
        if nhand != 2:
            return flat_feats
        traj_feats = einops.rearrange(
            flat_feats, 'b (l h) c -> b l h c', l=traj_len, h=nhand
        )
        traj_feats = self.apply_biroad(traj_feats)
        return einops.rearrange(traj_feats, 'b l h c -> b (l h) c')
    ...
    def forward(self, traj_feats, trajectory, timesteps,
        ...
        # Trajectory features
        if nhand > 1 and not self.no_hand_embed:
            traj_feats = traj_feats + self.hand_embed.weight[None, None]    # add hand identity embedding
        ...
        )[None, None].repeat(len(traj_feats), 1, nhand, 1)                  # add time embedding
        ...
        if self.should_apply_biroad("post_lang") and nhand == 2:
            traj_feats = self.apply_biroad_to_flat_features(
                traj_feats, traj_len, nhand
            )
        ...
        if self.should_apply_biroad("pre_self_attn") and nhand == 2:
            traj_feats = self.apply_biroad_to_flat_features(
                traj_feats, traj_len, nhand
            )
        ...
        # Apply BiRoAD to trajectory tokens after scene interaction.
        if self.should_apply_biroad("post_self_attn") and nhand == 2:
            # Extract only the trajectory features part
            b, total_len, c = features.shape
            traj_feats_flattened = features[:, :traj_len * nhand]
            updated_traj_feats_flattened = self.apply_biroad_to_flat_features(
                traj_feats_flattened, traj_len, nhand
            )
            features = torch.cat([
                updated_traj_feats_flattened,
                features[:, traj_len * nhand:]
            ], dim=1)

        # Rotation head
        rotation = self.predict_rot(
            features, rel_pos, time_embs, traj_feats.shape[1],
            traj_len=traj_len, nhand=nhand
        )
    ...
    def predict_pos(self, features, pos, time_embs, num_traj_tokens,
                    traj_len=None, nhand=None):
        ...
        position_features = position_features[:, :num_traj_tokens]
        if (
            traj_len is not None
            and nhand is not None
            and self.should_apply_biroad("position_head")
            and nhand == 2
        ):
            position_features = self.apply_biroad_to_flat_features(
                position_features, traj_len, nhand
            )
        position_features = self.position_proj(position_features)  # (B, N, C)
        position = self.position_predictor(position_features)
        return position, position_features

    def predict_rot(self, features, pos, time_embs, num_traj_tokens,
                    traj_len=None, nhand=None):
        ...
        rotation_features = rotation_features[:, :num_traj_tokens]
        if (
            traj_len is not None
            and nhand is not None
            and self.should_apply_biroad("rotation_head")
            and nhand == 2
        ):
            rotation_features = self.apply_biroad_to_flat_features(
                rotation_features, traj_len, nhand
            )
